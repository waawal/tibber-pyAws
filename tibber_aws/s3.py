...
class S3Bucket:
    ...
    async def load_data(self, key, if_unmodified_since=None):
        ...
        try:
            res = await (await self.client.get_object(Bucket=self._bucket_name,
                                                      Key=key,
                                                      IfUnmodifiedSince=if_unmodified_since))["Body"].read()
    ...
    async def store_data(self, key, data):
        resp = await self.client.put_object(Bucket=self._bucket_name,
                                            Key=key,
                                            Body=data)
        return resp
